...
export default function Post(){
  ...
  let dataSearch  = Postdata.postcardDate.filter(item => {
    return Object.keys(item).some(key => item[key].toString().toLowerCase().includes(filter.toString().toLowerCase()) )
  })
  const handleAlert = () => {
    alert('Post successfully added to the feed');
  }
  const handleAlertDel = () => {
    alert('Post removed from the feed');
  }
  const deletePost = (event) => {
  
    let Username = localStorage.getItem("user");
   
    axios.delete('http://localhost:80/api/posts.php/'+Username).then(function(response){
     ...
     <div className='row justify-content-center'>
         <div className='col-12 mb-5'>
             <div className='mb-3 col-4 mx-auto'>
                 <h1 className='form-label h4'>Search for Post</h1>
                 <input type='text' className='from-control' value={filter} onChange={searchText.bind(this)}/>
                 <br></br>
             <button className='btn1'><a href="/AddPost">add</a></button>
