...
export default function AddProduct(){
    ...
    const handleSubmit = (event) => {
        ...
        axios.post('http://localhost:80/api/products.php/', inputs).then(function(response){
            toast.success("Product added successfully.");
            console.log(response.data);
            navigate('/Products_Business');
    ...
    <div className="inputBox">
        ...
        <input className="size" type="text" name = 'ProductName' placeholder="eg: Pink_Jacket" onChange={handleChange} required/>
    </div>
    <div className="inputBox">
        <label>Price</label>
        <input className="size" type="number" name = 'Price' placeholder="eg: 60" onChange={handleChange} required/>
    </div>
